...
class UserRepository:
    ...
    def get_user_by_company_fkey(self, company_id_fkey):
        return (
            self._session.query(User).filter_by(company_id_fkey=company_id_fkey).first()
        )
    ...
    def does_exist(self, username):
        return self._session.query(User).filter_by(username=username).all()
    ...
    def get_all_user_roles(self, user_id):
        return (
            self._session.query(UserRoles).filter(UserRoles.user_fkey == user_id).all()
        )
