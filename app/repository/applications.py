...
class ApplicationsRepository:
    ...
    def find_application_by_id(self, id):
        return self._session.query(Applications).filter_by(id=id).first()
    ...
    def get_all_company_applications(self, company_id):
        ...
        #     return applications
        return (
            self._session.query(Applications)
            .join(Car)
            .join(Company)
            .filter(Company.id == company_id, Applications.is_archived == 0)
        ...
        if limit >= 0:
            if not is_superuser:
                return (
                    ...
                    .join(Car)
                    .join(Company)
                    .filter(Applications.is_archived == 0)
                    .order_by(
                        Applications.application_status.desc(),
                        Applications.date_applied.asc(),
                    )
                    .offset(offset)
                    .limit(limit)
                    .all()
    ...
    async def add_new_application(
        ...
        if folder_uuid and filename:
            self._session.add(
                Applications(
                    type_of_application=int(type_of_application),
    ...
    def get_application_count(self, is_superuser):
        if not is_superuser:
            return self._session.query(
                func.count(Applications.id).filter(Applications.is_archived == 0)
            ).all()
    ...
    def get_pending_applications_count(self):
        return len(
            self._session.query(Applications.id)
            .filter(
                Applications.application_status == "Pending",
                Applications.is_archived == 0,
            )
            .all()
        )
    ...
    def udpate_status(
        ...
        if is_accepted:
            ...
            if int(self.get_type_of_application(application_id)) == 3:
                car_id_of_deleted_car = self.get_car_id_from_application_id(
                    application_id
                )

                # get car ID from application ID, keep it DONE

                car_repo.update_car_internal_status(
                    int(car_id_of_deleted_car), "deleted"
                )
    ...
    def search_car_reg_like(self, car_reg, is_superuser=None):
        if is_superuser:
            return (
                ...
                .join(Car)
                .join(Company)
                .filter(Car.internal_car_reg_number.like(car_reg))
                .order_by(Applications.date_applied.desc())
                .all()
            )
        ...
        return (
            ...
            .join(Company)
            .filter(
                Applications.is_archived == 0, Car.internal_car_reg_number.like(car_reg)
            )
            .order_by(Applications.date_applied.desc())
            .all()
        )
    ...
    def archive_all_applications(self):
        self._session.query(Applications).filter(Applications.is_archived == 0).update(
            {Applications.is_archived: 1}, synchronize_session=False
        )
