...
class CarRepository:
    ...
    def find_by_car_reg(self, car_reg):
        return self._session.query(Car).filter_by(internal_car_reg_number=car_reg).first()


    def create_car_by_reg(self,
                          car_reg,
                          company_id,
                          internal_car_id=0,
    ...
    def get_all_similar_car_reg(self, car_reg):
        return self._session.query(Car).filter(Car.internal_car_reg_number.like(car_reg.upper())).all()
    ...
    def get_all_company_cars(self, company_id):
        return self._session.query(Car).filter(Car.company_id_fkey==int(company_id), Car.internal_car_status != 'deleted').all()


    def get_all_cars(self):
        return self._session.query(Car).order_by(Car.internal_car_reg_number.asc()).all()
