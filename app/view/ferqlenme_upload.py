...
def ferqlenme_upload():
    ...
    if all_cars and request.method == 'POST':
        ...
        this_controller = FerqlenmeUploaderController(car_repository, request,
        ...
        if this_controller.validate_car(all_cars) and this_controller.validate_file():
            ...
            is_uploaded_locally = this_controller.save_file_locally()  # returs bool or tuple -> folder_uuid, filename
            ...
            if is_uploaded_locally:
                ...
                io_loop.run_until_complete(this_controller.add_new_record_for_application(is_uploaded_locally[1]))

                # io_loop.run(this_controller.add_new_record_for_application(is_uploaded_locally[1]))

                # return success message

                return render_template("./index/ferqlenme_upload_form.html", title=title,
                ...
                return render_template("./index/ferqlenme_upload_form.html", title=title,
                                       fleet_name=session['internal_company_name'],
                                       internal_company_balance_ref=session['internal_company_balance_ref'],
                                       all_cars=all_cars, is_uploaded=False,
                                       keyword=session["keyword"])
    ...
    # GET request
    return render_template("./index/ferqlenme_upload_form.html", title=title, fleet_name=session['internal_company_name'],
                           internal_company_balance_ref=session['internal_company_balance_ref'], all_cars=all_cars,
                           keyword=session["keyword"])
