...
def admin_index():
    ...
    is_approved = (
        ...
        is_archived
    ) = is_archived_all = list_of_updated_applications = updated_application = None
    ...
    if request.method == "POST":
        ...
        if this_controller.is_approve_request():
            updated_application = this_controller.update_application_status(
                is_accepted=True
            )
            is_approved = True
            ...
            list_of_updated_applications = this_controller.update_application_status(
                is_archived=True
            )
            # print(request.form.getlist('applicationIDs[]'))
            is_archived = True
    ...
    # GET validation
    if this_controller.validate_limit(
        is_superuser=True if 1 in session["all_user_roles"] else None
    ):
        try:
            all_applications = applications_repository.get_all_applications_bulk(
                15,
                int(limit) * 15,
                is_superuser=True if 1 in session["all_user_roles"] else None,
            )
            ...
            signed_urls = this_controller.get_gcloud_urls(
                all_applications, car_repository, user_repository
            )
        ...
        pending_applications_count = (
            applications_repository.get_pending_applications_count()
        )

        # print(applications_count)

        return render_template(
            "./admin/admin_index_new_table.html",
            title=title,
            fleet_name=session["internal_company_name"],
            internal_company_balance_ref=session["internal_company_balance_ref"],
            application_count=pending_applications_count,
            all_applications=all_applications,
            current_limit=int(request.args.get("limit")),
            ...
            list_of_updated_applications=list_of_updated_applications,
...
def admin_create_new_user():
    ...
    list_of_all_companies = Admin.get_all_available_companies()

    map_users_companies = Admin.map_available_companies_with_users(
    ...
    if request.args.get("isSuccess") == "True":
        if request.args.get("isCompany") == "True":
            return render_template(
                "./admin/admin_create_new_user.html",
                title=title,
                fleet_name=session["internal_company_name"],
                internal_company_balance_ref=session["internal_company_balance_ref"],
                is_success=True,
                company_name=HelperUtils.try_or_default(
                    lambda: Admin.get_company_by_id(
                        request.args.get("companyId")
                    ).internal_company_name,
                    "",
                ),
                company_models=map_users_companies,
                reversed_keys=list(reversed(list(map_users_companies.keys()))),
                keyword=session["keyword"],
            )
            ...
            return render_template(
                ...
                internal_company_balance_ref=session["internal_company_balance_ref"],
                is_success=True,
                user_name=Admin.get_user_by_id(request.args.get("userId")).username,
                company_models=map_users_companies,
                reversed_keys=list(reversed(list(map_users_companies.keys()))),
                keyword=session["keyword"],
            )
        ...
        if request.args.get("isCompany") == "True":
            return render_template(
                ...
                internal_company_balance_ref=session["internal_company_balance_ref"],
                is_success=False,
                error_message="Bu şirkət artıq mövcuddur.",
                company_models=map_users_companies,
                reversed_keys=list(reversed(list(map_users_companies.keys()))),
                keyword=session["keyword"],
            )
            ...
            return render_template(
                ...
                internal_company_balance_ref=session["internal_company_balance_ref"],
                is_success=False,
                error_message=request.args.get("errorMsg"),
                company_models=map_users_companies,
                reversed_keys=list(reversed(list(map_users_companies.keys()))),
                keyword=session["keyword"],
            )
...
def admin_create_new_userr():
    ...
    if input_ := controller.validate_user_creation(
        ...
        if isinstance(input_, User):
            ...
            user_repository.create_a_user(input_)
...
def admin_create_new_company():
    ...
    if input_ := controller.validate_company(
        ...
        if not controller.is_company_already_exists(company_name):
            controller.create_company(
                company_name, internal_company_id, balance_ref_number
            )
            ...
            return redirect(
                url_for("admin.admin_create_new_user", isSuccess=False, isCompany=True)
            )
...
def search():
    ...
    if x:
        applications_like_car_reg = super_user_wrapper(
            applications_repository.search_car_reg_like, f"%{x.upper()}%"
        )
        ...
        if applications_like_car_reg:
            ...
            all_apps = super_user_wrapper(
                applications_repository.get_all_applications_bulk, 15, int(limit) * 15
            )
    ...
    if x:
        ...
        if similar_cars:
            ...
            return jsonify(
                {
                    "result": [
                        {0: i.internal_car_reg_number, 1: i.id} for i in similar_cars
                    ]
                }
            )
...
def download_csv():
    ...
    data = pd.read_sql_query(
        "SELECT * FROM applications JOIN cars c on c.id = applications.car_id_fkey JOIN company c2 on c2.id = c.company_id_fkey;",
        engine,
    )

    csv_data = data.to_csv(
        path_or_buf="./tmp/downloaded_files/export.csv", index=False, mode="w"
    )
...
def super_user_wrapper(fn, *args):
    if 1 in session["all_user_roles"]:
        if fn.__name__ != "search_car_reg_like":
            return fn(-1, 0, is_superuser=True)
        else:
            return fn(*args, is_superuser=True)
