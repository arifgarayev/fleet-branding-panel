...
class UserController:
    ...
    def set_session(self, session_name: str, session_data):
        session[session_name] = session_data

    def validate_username_and_password(
        self, user_input_password, user_input_username
    ) -> bool:
        # self.user_model = True <- already checks if user exists or not
        if (
            self.user_model
            and bcrypt.check_password_hash(
                self.user_model.password, user_input_password
            )
            and len(user_input_password) < 50
            and self.validate_username(user_input_username)
        ):
            ...
            comnpany_joined_model = self.user_repository.join_with_company(
                current_user.id
            )
            ...
            ] = comnpany_joined_model.internal_company_name
            session[
                "internal_company_status"
            ] = comnpany_joined_model.internal_company_status
            session[
                "internal_company_balance_ref"
            ] = comnpany_joined_model.internal_company_balance_ref
            session["is_admin"] = self.user_model.is_admin
            session["all_user_roles"] = [
                int(i.role_fkey)
                for i in self.user_repository.get_all_user_roles(self.user_model.id)
            ]
            # session["keyword"] = [
            #     {}
            # ]
    ...
    def validate_username(self, user_input_username):
        ...
        if (
            self.user_model
            and len(user_input_username) < 50
            and not user_input_username.isdigit()
        ):
            return True
