...
class FerqlenmeUploaderController:
    ...
    def __init__(self, car_repository, request, applications_repo):
        ...
        try:
            self.req_car_reg_input_t = int(
                    self.request.form['isCustomCarReg']
            )
    ...
    def disallowed_file(self, filename):
        return '.' in filename and filename.rsplit('.', 1)[
            1].lower() in self.DISALLOWED_EXTENSIONS

    def validate_file(self):
        return True if 'file' in list(self.request.files.keys())[
            0] and not self.disallowed_file(self.file.filename) else False
    ...
    async def add_new_record_for_application(
            self, filename, type_of_application=1
    ):
