...
from app.view.video_upload import car_repository, company_repository, \
    applications_repository, user_repository
...
class Admin:
    ...
    def validate_limit(self,
                       is_superuser):
        try:
            applications_count = applications_repository.get_application_count(is_superuser=is_superuser)

            if int(self.request.args.get("limit"))  > int(applications_count[0][0]) // 15 or \
                    int(self.request.args.get("limit")) < 0:
                return False
            else:
                return True
    ...
    def get_gcloud_urls(self, application_model, car_repo, user_repo):
        return VideoUploaderService.get_signed_urls(application_model, car_repo, user_repo)
    ...
    def update_application_status(self, is_accepted=None,
                                  is_declined=None,
                                  is_revert=None,
                                  is_archived=None,
        ...
        if is_accepted:
            application = applications_repository.udpate_status(int(self.request.form.get('approve')), is_accepted=is_accepted, car_repo=car_repository)
            self.set_comment(self.request.form.get('approve'))

        elif is_declined:
            application = applications_repository.udpate_status(int(self.request.form.get('reject')), is_declined=is_declined)
            self.set_comment(self.request.form.get('reject'))

        elif is_revert:
            application = applications_repository.udpate_status(int(self.request.form.get('revert')), is_revert=is_revert)
            self.set_comment(self.request.form.get('revert'), is_revert=True)

        elif is_archived:
            # print(self.request.form.getlist('applicationIDs[]'))
            list_of_archived_applications = applications_repository.udpate_status(self.request.form.getlist('applicationIDs[]'), is_archived=True)
        ...
        if application:
            ...
            company_id_based_on_car_id = car_repository.get_company_fkey_by_car_id(int(self.request.form.get('car_ref_' + str(application.id))))
            self.update_appropriate_car(int(self.request.form.get('car_ref_' + str(application.id))), int(application.id), int(company_id_based_on_car_id))
    ...
    def update_appropriate_car(self, car_id_pkey, application_id, company_id_fkey):
        ...
        applications_repository.update_applications_car(car_id_pkey, application_id, user_model)
    ...
    def validate_company(self, company_name,
        ...
        if company_name.isascii():
            ...
            company_name = company_name[0:2].upper() + ' ' + company_name[3:].capitalize()
    ...
    def map_available_companies_with_users(list_of_all_company_models):
        ...
        all_users_fkey = [user.company_id_fkey for user in user_repository.get_all_users()]
        ...
        for c in list_of_all_company_models:
            ...
            if c.id in all_users_fkey else 0
            }
    ...
    def validate_user_creation(self,
                               ...
                               email = None,
                               mobile = None,
                               internal_manager_id = None) -> str | User:


        if user_repository.does_exist(
                username.casefold()
        ):
            return 'This username already exists in our ' \
                   'Database'
        ...
        try:
            ...
            if internal_manager_id:
                ...
                my_user.set_internal_manager_id(
                    internal_manager_id
                )
